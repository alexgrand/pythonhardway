...
json_text = json.dumps(all_scenes)
normal_text = json.loads(json_text)
print(normal_text)
print("*" * 210)
print(all_scenes)
print(all_scenes == normal_text)
print("*" * 210)
...
class Raw_Scene(object):
  ...
  def put_text(self):
    ...
    while i <= len(words_list):
      ...
      while len(line) < max_line_size and x <= len(words_list):
        line = ' '.join(words_list[i:x])
        x += 1
        
      new_line = ' '.join(words_list[i:x])
      
      print(new_line)

      new_line += "\n\t"
      new_line = new_line.replace("*n**t*", "\n\t")
      
      text_line['text'] += new_line
